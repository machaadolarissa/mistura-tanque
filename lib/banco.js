...
async function adicionarPh(valor, criadoEmOpcional) {
  const criadoEm = criadoEmOpcional
    ? new Date(criadoEmOpcional).toISOString()
...
async function listarPh(limite = 20) {
  if (usandoTurso()) {
    const rows = await tursoExecutar(
      "SELECT ph, criado_em AS criadoEm FROM ph ORDER BY criado_em DESC LIMIT ?",
      [limite]
    );
    return rows.map((r) => ({
      ph: Number(r.ph),
